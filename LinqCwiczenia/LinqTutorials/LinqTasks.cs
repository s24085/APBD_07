...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            var result = Emps.Select(emp => new { Nazwisko = emp.Ename, Praca = emp.Job });
            return result;
        }
        ...
        {
            var result = Emps.Select(emp => new { emp.Ename, emp.Job, emp.HireDate })
        ...
        {
            var result = arr.GroupBy(x => x).Single(g => g.Count() % 2 != 0).Key;
            return result;
        }
        ...
        {
            var result = Depts.Where(dept => Emps.Count(emp => emp.Deptno == dept.Deptno) == 5 || !Emps.Any(emp => emp.Deptno == dept.Deptno))
        ...
        {
            var result = Emps.SelectMany(emp => Depts, (emp, dept) => new { Emp = emp, Dept = dept });
            return result;
        }
    }
